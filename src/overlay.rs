...
pub(crate) enum Src {
    ...
    Base { origin: RelPath, id: FileId, mode: u32 },
...
impl Overlay {
    pub fn new(root_id: FileId, case_insensitive: bool) -> Overlay {
        Overlay { placed: BTreeMap::new(), detached: BTreeMap::new(), root_id, case_insensitive, next_dir: 0 }
    }

    pub fn root_entry(&self) -> Entry {
        Entry { kind: EKind::Dir, src: Src::Base { origin: RelPath::root(), id: self.root_id, mode: 0 } }
    }
    ...
    fn base_lookup(&self, vfs: &dyn Vfs, b: &RelPath) -> Result<Option<Entry>> {
        ...
        if self.case_insensitive {
            ...
            if !names.iter().any(|n| n == name) {
                ...
                if let Some(other) = names.iter().find(|n| crate::path::os_bytes(n).to_ascii_lowercase() == folded) {
                    return Err(Error::CaseCollision { path: b.to_path_buf(), existing: parent.join(other.clone()).to_path_buf() });
                }
                return Ok(None);
            }
        }
        Ok(vfs.stat(b)?.map(|m| Entry { kind: ekind(m.kind), src: Src::Base { origin: b.clone(), id: m.id, mode: m.mode } }))
    }
    ...
    pub fn list(&self, vfs: &dyn Vfs, dir: &RelPath, e: &Entry) -> Result<BTreeSet<OsString>> {
        ...
        if let Src::Base { origin, .. } = &e.src {
            for n in vfs.read_dir(origin)? {
                ...
                if v.is_private() || self.placed.contains_key(&v) || self.detached.contains_key(&origin.join(n.clone())) {
                    continue;
                }
                names.insert(n);
            }
    ...
    fn check_new_name(&self, vfs: &dyn Vfs, p: &RelPath, parent: &Entry) -> Result<()> {
        ...
        for n in self.list(vfs, &dir, parent)? {
            if crate::path::os_bytes(&n).to_ascii_lowercase() == folded {
                return Err(Error::CaseCollision { path: p.to_path_buf(), existing: dir.join(n).to_path_buf() });
            }
        }
        Ok(())
    }
    ...
    pub fn check_write(&self, vfs: &dyn Vfs, p: &RelPath) -> Result<Option<u32>> {
        ...
        match self.resolve(vfs, p)? {
            Some(Entry { kind: EKind::Dir, .. }) => Err(Error::IsADirectory(p.to_path_buf())),
            Some(Entry { kind: EKind::Special, .. }) => Err(Error::UnsupportedFileType(p.to_path_buf())),
            Some(Entry { src: Src::Base { mode, .. }, .. }) => Ok(Some(mode)),
            Some(Entry { src: Src::Blob { mode, .. }, .. }) => Ok(mode),
            Some(Entry { src: Src::NewDir { .. }, .. }) => unreachable!("new dirs are dirs"),
            None => {
    ...
    pub fn apply_write(&mut self, vfs: &dyn Vfs, p: &RelPath, n: u32, mode: Option<u32>) -> Result<()> {
        if let Some(e) = self.resolve(vfs, p)? {
            self.detach(&e);
        }
        self.placed.insert(p.clone(), Slot::Present(Entry { kind: EKind::File, src: Src::Blob { n, mode } }));
        Ok(())
    }
        ...
        for q in p.proper_prefixes().chain(std::iter::once(p.clone())) {
            match self.resolve(vfs, &q)? {
                ...
                None => {
                    ...
                    let n = self.next_dir;
                    self.next_dir += 1;
                    self.placed.insert(q, Slot::Present(Entry { kind: EKind::Dir, src: Src::NewDir { n } }));
                }
            }
        }
        Ok(())
    ...
    pub fn rename(&mut self, vfs: &dyn Vfs, from: &RelPath, to: &RelPath) -> Result<()> {
        let e = self.resolve(vfs, from)?.ok_or_else(|| Error::NotFound(from.to_path_buf()))?;
        ...
        if from.is_ancestor_of(to) {
            return Err(Error::InvalidMove { from: from.to_path_buf(), to: to.to_path_buf() });
        }
    ...
    pub fn remove(&mut self, vfs: &dyn Vfs, p: &RelPath) -> Result<()> {
        let e = self.resolve(vfs, p)?.ok_or_else(|| Error::NotFound(p.to_path_buf()))?;
        match e.kind {
    ...
    pub fn read(&self, vfs: &dyn Vfs, p: &RelPath, blob_path: impl Fn(u32) -> RelPath) -> Result<Vec<u8>> {
        match self.resolve(vfs, p)? {
            None => Err(Error::NotFound(p.to_path_buf())),
            Some(Entry { kind: EKind::Dir, .. }) => Err(Error::IsADirectory(p.to_path_buf())),
            Some(Entry { kind: EKind::Special, .. }) => Err(Error::UnsupportedFileType(p.to_path_buf())),
            Some(Entry { src: Src::Base { origin, .. }, .. }) => Ok(vfs.read_file(&origin)?),
            Some(Entry { src: Src::Blob { n, .. }, .. }) => Ok(vfs.read_file(&blob_path(n))?),
            Some(Entry { src: Src::NewDir { .. }, .. }) => unreachable!(),
        }
    }
}
